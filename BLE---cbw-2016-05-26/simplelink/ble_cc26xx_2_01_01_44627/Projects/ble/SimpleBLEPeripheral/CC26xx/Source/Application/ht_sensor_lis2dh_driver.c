...
#define SENSOR_SELECT()     bspI2cSelect(BSP_I2C_INTERFACE_0,LIS2DH_MEMS_I2C_ADDRESS)

status_t LIS2DH_Init(void)
{
  uint8_t ret;
  bspI2cInit();
  ret = LIS2DH_SetODR(LIS2DH_ODR_100Hz);
  if(!ret){
    HalLedSet(HAL_LED_2, HAL_LED_MODE_FLASH |(2<<4));
    pr_err("setODR err  %d\n",ret);
    return MEMS_ERROR;
  }
  ret = LIS2DH_SetMode(LIS2DH_NORMAL);
  if(!ret){
    HalLedSet(HAL_LED_2, HAL_LED_MODE_FLASH |(2<<4));
    pr_err("SetMode err  %d\n",ret);
    return MEMS_ERROR;
  }
  ret = LIS2DH_SetFullScale(LIS2DH_FULLSCALE_2);
  if(!ret){
    HalLedSet(HAL_LED_2, HAL_LED_MODE_FLASH |(2<<4));
    pr_err("SetFullScale err  %d\n",ret);
    return MEMS_ERROR;
  }
  ret = LIS2DH_SetAxis(LIS2DH_X_ENABLE | LIS2DH_Y_ENABLE | LIS2DH_Z_ENABLE);
  if(!ret){
    HalLedSet(HAL_LED_2, HAL_LED_MODE_FLASH |(2<<4));
    pr_err("SetAxis err  %d\n",ret);
    return MEMS_ERROR;
  }
  return MEMS_SUCCESS;
}
...
u8_t LIS2DH_ReadReg(u8_t Reg, u8_t* Data) {
  ...
  SENSOR_SELECT();  
  return sensorReadReg(Reg,Data,2);
}
...
u8_t LIS2DH_WriteReg(u8_t WriteAddr, u8_t Data) {
  ...
  SENSOR_SELECT();
  return sensorWriteReg(WriteAddr,&Data,1);
}
